...
sap.ui.define([
    ...
    return Controller.extend("com.bts.zbts.controller.App", {
        onInit: function () {
            // Chỉ gọi để lấy thông tin Username hiển thị cho đẹp, không chặn quyền nữa
            this._getPublicInfo();
        },

        _getPublicInfo: function() {
            var oDataModel = this.getOwnerComponent().getModel();
            var oUserModel = this.getOwnerComponent().getModel("userInfo");
            var oRouter = this.getOwnerComponent().getRouter();
            var sPath = "/UserRoleSet('Current')"; 
            ...
            oDataModel.read(sPath, {
                success: function(oData) {
                    ...
                    // Set tất cả quyền thành TRUE cho mọi người
                    oUserModel.setData({
                        username: oData.Username,
                    ...
                    // Luôn luôn điều hướng về trang quản lý dự án
                    oRouter.navTo("RouteProjectManagement");
                },
                error: function(oError) {
                    sap.ui.core.BusyIndicator.hide();
                    // Nếu lỗi Backend vẫn cho vào trang chính để làm việc offline hoặc test UI
                    oRouter.navTo("RouteProjectManagement");
                }
            });
        }
